...
def run_one_report(air, dev):
    ...
    try:
        ...
        export_file = os.path.dirname(os.getcwd()) + '\\static\\report\\' + dev + '_' + datetime.date.today().strftime(
            '%Y%m%d')
        print(export_file)
        ...
        if os.path.isfile(log):
            ...
            cmd = [
                ...
                log_dir,
                "--static_root",
                "http://127.0.0.1:8000/static/airtest",
                "--lang",
                "zh",
                "--export",
                export_file,
            ...
            return {
                'status': ret,
                'path': os.path.join(export_file, air[:-4] + '.log\\log.html')
            }
        else:
...
def run_summary(data):
    ...
    try:
        ...
        rep = 'http:\\127.0.0.1:8000\\static\\report\\' + "report.html"
...
# 程序调用接口，使用异步调用
async def execute_case(select_devices, case_list):
    for case in case_list:
        run(select_devices, case, run_all=True)
...
if __name__ == '__main__':
    ...
    # asyncio.run(execute_case(devices, air))
